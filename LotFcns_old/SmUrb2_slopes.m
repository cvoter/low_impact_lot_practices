function [slopeX,slopeY] = SmUrb2_slopes(x,nx,dx,y,ny,dy,fc,parcelCover,downspout,landSlope,transverseSlope,dsLength)
...
for i = 1:ny
    ...
    for j = 1:nx
        ...
        elseif thisY >= ymidHouse && parcelCover(i,j) < 7
            %Back Yard
            slopeY(i,j) = -landSlope;
            if (downspout == 3) && (thisY >= fc(7,4)-3*dy) &&...
                    (thisY <= fc(7,4)-2*dy) && thisX < fc(7,1)
                slopeY(i,j) = 0;
            elseif (downspout == 3) && (thisY <= fc(9,3)+3*dy) &&...
        ...
        elseif thisX > fc(7,2) && thisY > fc(1,4) &&...
            ...
            if thisX > fc(9,1) && thisX < fc(9,2) && thisY > fc(9,4) && thisY < fc(2,3)
                slopeX(i,j) = 0; %zero if behind the garage
            end
