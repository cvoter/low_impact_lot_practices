function [slopeX,slopeY] = LotC_slopes(x,nx,dx,y,ny,dy,fc,parcelCover,triggers,details)
...
%% DEVELOPED SLOPES
if microType == 1
    load('J:\Research\Parflow\inputs\matlab_in\LotFcnsABC\LotC_microelev.mat');
else
    slopeX = zeros([ny nx]);
    slopeY = zeros([ny nx]);
end
for i = 1:ny
    ...
    for j = 1:nx
        ...
        elseif parcelCover(i,j) == 5
            slopeY(i,j) = landSlope;
        elseif thisY < (fc(1,4)-2*dy) || ( (thisY > fc(2,3)) && (thisY < (fc(2,3)+dy)) )
        ...
        elseif thisY >= ymidHouse && parcelCover(i,j) < 7
            %Back Yard
            slopeY(i,j) = -landSlope;
            if (downspout == 0) && (thisY >= fc(7,4)-3*dy) &&...
                    (thisY <= fc(7,4)-2*dy) && thisX < fc(7,1)
                slopeY(i,j) = 0;
            elseif (downspout == 0) && (thisY <= fc(9,3)+3*dy) &&...
                    (thisY >= fc(9,3)+2*dy) && thisX > fc(9,2)
                slopeY(i,j) = 0;
            end
            if (thisX >= (fc(5,1)-dx)) && (thisX < fc(5,1)) && (thisY <= fc(9,3))
        ...
        if (downspout == 1) && thisY < fc(9,3) && (thisY >= (fc(9,3)-dsLength))...
                && (thisX > (fc(9,2)-2*dx)) && (thisX < (fc(9,2)-dx)) && parcelCover(i,j) ~= 5
            %Downspout = 1, Back Garage
            slopeX(i,j) = 0;
        elseif (downspout == 1) && (thisY < (fc(9,4)-dy)) && (thisY > (fc(9,4)-2*dy))...
                && (thisX > (fc(9,1)-dsLength)) && (thisX < fc(9,1))
        ...
        elseif thisX > fc(7,2) && thisY > fc(1,4) &&...
            ...
            slopeX(i,j) = -landSlope;
            if thisX > fc(9,1) && thisX < fc(9,2) && thisY > fc(9,4) && thisY < fc(2,3)
                %Behind garage
                slopeX(i,j) = 0;
            end
            if (thisX >= (fc(5,1)-dx)) && (thisX < fc(5,1)) && (thisY <= fc(9,3))
        ...
        elseif (thisY >= (fc(9,3)-2*dy)) && (thisY < fc(9,3)) &&...
                (thisX >= (fc(9,1)-dx)) && (thisX < fc(9,2)) && parcelCover(i,j) ~= 5
            %Below Garage
            slopeX(i,j) = -landSlope;
        elseif (thisY < fc(1,4) || thisY > fc(2,3)) && thisX < (fc(1,2)/2)
            %Street & Alley
            slopeX(i,j) = landSlope;
        elseif (thisY < fc(1,4) || thisY > fc(2,3)) && thisX >= (fc(1,2)/2) && parcelCover(i,j) ~= 5
            %Street & Alley & Driveway
            slopeX(i,j) = -landSlope;
        end
    end
