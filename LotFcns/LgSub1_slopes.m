function [slopeX,slopeY] = LgSub1_slopes(x,nx,dx,y,ny,dy,fc,parcelCover,downspout,landSlope,transverseSlope,dsLength)
...
for i = 1:ny
    ...
    for j = 1:nx
        ...
        elseif downspout == 2
            ...
            if parcelCover(i,j) == 9 %Garage X-slopes
                if thisX > fc(9,1)+2*dx
                    slopeX(i,j) = roofSlope;
                elseif thisX <= fc(9,1)+dx
                    slopeX(i,j) = -roofSlope;
                end
            elseif parcelCover(i,j) == 7 %House X-slopes
